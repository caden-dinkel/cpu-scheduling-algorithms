...
const FTCFStep = (myState: AlgorithmState) => {
  ...
  //Preemptive
  //Sort by remaining burst time ascending
  if (newReadyQueue.at(0) !== undefined) {
    newReadyQueue.sort(
      (a, b) =>
        a.burstTime -
        a.remainingBurstTime -
        (b.burstTime - b.remainingBurstTime)
    );
  }

  //If there is a shorter process in queue, remove process from execution
  //Add time segment to AlgorithmExecution
  if (
    newReadyQueue.at(0) &&
    newExecutingProcess.at(0) &&
    newReadyQueue[0].burstTime - newReadyQueue[0].remainingBurstTime <
